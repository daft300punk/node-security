...
class TimelineComponent extends Component {
  constructor(props) {
    ...
    this.getTimelinOptions = this.getTimelinOptions.bind(this);
  ...
  updateTimeline(timelineItems, timelineOptions) {
    ...
    const options = this.getTimelinOptions(timelineOptions);

    this.timeline.setOptions(options);
    this.timeline.setItems(data);
  }

  getTimelinOptions(options) {
    var timelineOptions = {
      width: "100%",
      editable: false,
      type: "box",
  ...
  createTimeline() {
    ...
    const timelineOptions = this.getTimelinOptions();
